...
function [bl,bu] = aux_fourierMotzkinApproximation(A,b,options)
    ...
    while tighterBnds && iter <= maxIter
        ...
        tmp = repmat(bl_,1,size(A,1),1);
        tmp(A_ < 0) = rh(A_ < 0);
        bl_ = max(tmp,[],2);
        % Update the upper bounds.
        tmp = repmat(bu_,1,size(A,1),1);
        tmp(A_ > 0) = rh(A_ > 0);
        bu_ = min(tmp,[],2);
...
function h = aux_boundConZonotopeSupportFunction(A,b,G,options)
    ...
    %
    % [Lower Bound]     l_{(i)} = c_{(i)} + max -G_{(i,.)}\,\beta 
    %                                       s.t. A\,\beta <= b
    %                                              -\beta <= 1
    %                                               \beta <= 1
    %
    % [Upper Bound]     u_{(i)} = c_{(i)} + max G_{(i,.)}\,\beta 
    %                                       s.t. A\,\beta <= b
    %                                              -\beta <= 1
    %                                               \beta <= 1
    %
    ...
    % \beta, i.e., -1 <= \beta <= 1.)
    %
    % [Lower Bound]     min b^T\,\lambda + 1^T\,(\mu + \nu)
    %                   s.t. A^T\,\lambda - \mu + \nu = -G_{(i,.)}^T
    %                               \lambda >= 0, \mu >= 0, \nu >= 0
    % 
    % [Upper Bound]     min b^T\,\lambda + 1^T\,(\mu + \nu)
    %                   s.t. A^T\,\lambda - \mu + \nu = G_{(i,.)}^T
    %                               \lambda >= 0, \mu >= 0, \nu >= 0
    %
    ...
    %
    % [Lower Bound] -\mu + \nu = -G_{(i,.)}^T - A^T\,\lambda, and 
    %
    % [Upper Bound] -\mu + \nu = G_{(i,.)}^T - A^T\,\lambda. 
    %
    % Therefore, \mu and \nu are optimized by 
    %
    % [Lower Bound]    min 1^T\,(\mu + \nu) = 1^T\,\abs(-G_{(i,.)}^T - A^T\,\lambda)\,1
    %                  s.t. \mu,\nu >= 0                
    % 
    % [Upper Bound]    min 1^T\,(\mu + \nu) = 1^T\,\abs(G_{(i,.)}^T - A^T\,\lambda)\,1
    %                  s.t. \mu,\nu >= 0
    %
    % The optimal objective is \abs{+/-G_{(i,.)}^T - A^T\,\lambda}\,1.
    ...
    maxStepSize = options.nn.conzonotope_bound_step_size;
    ...
    stepSizes = permute((beta.^(0:(numStepSizes-1)))*maxStepSize,[1 3 4 2]);
    ...
    sAbs = @(x) sum(sqrt(x.^2 + epsilon^2) - epsilon,2);
    dsAbs = @(x) x./sqrt(x.^2 + epsilon^2);

    % Initialize the dual variables.
    lambda = zeros([p n bSz],'like',G);
    % Initialize iteration counter.
    iter = 1;

    % Initialize FISTA parameters.
    y = lambda; % Extrapolation point
    t = 1; % Acceleration parameter

    % Do an iterative optimization of the dual variables.
    while iter <= maxIter
        % Compute the gradient.
        G_yA = G - pagemtimes(y,'transpose',A,'none');
        dfy = b - pagemtimes(A,'none',dsAbs(G_yA),'transpose');

        % Do a backtracking line search to find a suitable step size: Find 
        % the largest step size that satisfies the Armijo–Goldstein 
        % condition. Therefore, we do a simultaneous step with all step
        % sizes and pick the best result.

        % Compute objective at current point.
        fy = reshape(pagemtimes(y,'transpose',b,'none') ...
            + sAbs(G_yA),[n bSz]);
        ...
        G_zA = G - pagemtimes(z,'transpose',A,'none');
        fz = reshape(pagemtimes(z,'transpose',b,'none') ...
            + sAbs(G_zA),[n bSz numStepSizes]);
        % Compute the Armijo–Goldstein condition.
        diff = z - y;
        qzy = fy + reshape( ...
            sum(diff.*dfy,1) + (1/(2*stepSizes)).*sum(diff.^2,1), ...
                [n bSz numStepSizes]);
        ...
        idx = sub2ind([n bSz numStepSizes], ...
            repelem((1:n)',1,bSz),repmat(1:bSz,n,1),ids);
        z = reshape(z(:,idx),[p n bSz]);

        % Compute the next acceleration parameter.
        t_ = (1 + sqrt(1 + 4*t^2))/2;
    ...
    sA = G - pagemtimes(lambda,'transpose',A,'none');
    h = reshape(pagemtimes(lambda,'transpose',b,'none') ...
        + sum(abs(sA),2),[n bSz]);
end
